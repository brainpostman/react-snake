...
export default function Snake(props) {
    ...
    const checkWallContact = (head) => {
        if (head[0] === 1 || head[0] === (gridHeight - 2) || head[1] === 1 || head[1] === (gridWidth - 2)) {
            setLoss(true);
        }
    }
    ...
    return (
        <div className="snake-container">
                        ...
                        <div className={"grid-container"} style={{gridTemplateColumns: `repeat(${gridWidth}, 1fr)`}}>
                            {
                                gridState.map((itemH, indexH) => {
                                    return itemH.map((itemW, indexW) => {
                                        switch (itemW) {
                                            case 1:
                                                return <div key={`h${indexH} w${indexW}`} className="cell-raised snake">
                                                </div>
                                            case 2:
                                                return <div key={`h${indexH} w${indexW}`} className="cell-raised food">
                                                </div>
                                            case 3:
                                                return <div key={`h${indexH} w${indexW}`} className="cell-raised wall">
                                                </div>
                                            default:
                                                return <div key={`h${indexH} w${indexW}`} className="cell empty">
                                                </div>
                                        }
                                    })
                                }).flatMap((item) => {
            ...
            { (start && !loss) && <div className="score"><p>SCORE:</p><p>{score}</p></div>}
        </div>
    );
}